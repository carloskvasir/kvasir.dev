...
export class FeedbackUtils {
  ...
  static playSound(type: 'success' | 'error' | 'click' | 'notification' = 'click') {
  ...
  static feedback(type: 'success' | 'error' | 'warning' | 'tap' = 'tap', options: { 
    enableSound?: boolean, 
    ...
    if (enableSound) {
      const soundType = type === 'tap' ? 'click' : type === 'warning' ? 'notification' : type
      this.playSound(soundType as 'success' | 'error' | 'click' | 'notification')
    }
  }

  // Efeito de pulse visual
  static pulseElement(element: HTMLElement, intensity: 'light' | 'medium' | 'strong' = 'medium') {
    if (!element) return
  ...
  // Efeito ripple
  static rippleEffect(event: React.MouseEvent<HTMLElement>, color: string = 'rgba(255, 255, 255, 0.3)') {
